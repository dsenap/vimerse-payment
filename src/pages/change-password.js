...
const rePass = /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9]).{8,}$/;
  const validationSchema = Yup.object().shape({
    ...
    password: Yup.string().min(8,"Password should contain at least 8 characters").matches(rePass,"Should contain 1 numeric and 1 uppercase letter and 1 lowercase letter").required('Password is required'),
  ...
  const submitForm = async (value) => {
    try{
      ...
      const data = await axios
        .post(`${process.env.NEXT_PUBLIC_baseUrl}users/reset-password`, {
