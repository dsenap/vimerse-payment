...
import { Container, Button, TextField, Grid, Typography, Checkbox ,InputAdornment,IconButton } from '@material-ui/core';
...
const Profile = () => {
  ...
  const getUserProfileData = async () => {
    ...
    let query = user.id||user._id;
    const data = await axios
      .get(`${process.env.NEXT_PUBLIC_baseUrl}users/byid?id=` + query, {
              ...
              {userData &&
                <form onSubmit={handleSubmit(onSubmitHandler)}>
                  ...
                  <Grid container className={`${s.containeritem} ${s.checkbox}`} direction="row" spacing={2}>
                    <Grid item>
                      <Checkbox defaultChecked />I would like to receive email notifications about updates and commercial
                      offers from Vimerse.
                    </Grid>
                  </Grid>
