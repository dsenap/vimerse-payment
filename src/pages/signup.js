...
const SignUp = () => {
  ...
  const submitForm = async (value) => {
    try {
      const data = await axios.post(`${process.env.NEXT_PUBLIC_baseUrl}users/signup`, {
        email: value.email,
  ...
  const responseGoogle = async (response) => {
    try {
      const data = await axios.post(`${process.env.NEXT_PUBLIC_baseUrl}users/signup/google`, {
  ...
  const responseGoogleSignin = async (response) => {
    try {
      const data = await axios.post(`${process.env.NEXT_PUBLIC_baseUrl}login/google`, {
        token: response.tokenId,
      });
      if (data && data.status === 200) {
        console.log('data.data', data.data);
      ...
      if (error.response) {
        ...
        } else if (toastrMessage === 'Incorrect Password')
              ...
              <Paper className={s.papercarde}>
                <center>
                  ...
                  <Typography variant="body">
                    We sent you an account verification email to the registered email address. If you do not see the
                    verification email, please wait a few minutes or check the spam folder.
                  </Typography>
                </center>{' '}
              </Paper>
            ) : (
